...
/* SLAVE_ADDRESS:
   - 0x0C: for S625 and S628
   - 0x0E: for S623 with CAD0=Low
   - 0x0F: for S623 with CAD0=High  */
...
#define FORCE_SENSORS_LAYOUT    0       //0:auto, 1:old, 2:new

/*----------------------------------------------------------------------------*/
struct mag_hw* s62x_get_cust_mag_hw(void);
#define get_cust_mag_hw()       s62x_get_cust_mag_hw()
...
#define S62X_I2C_ADDRESS        UMAP(ALTER_ADDRESS)
#else
#define S62X_I2C_ADDRESS        UMAP(SLAVE_ADDRESS)
#endif
...
#define S62X_IDX_GAIN_X         0x13
#define S62X_IDX_GAIN_Y         0x14
#define S62X_IDX_GAIN_Z         0x15
#define GAIN_DEFAULT            0x09

#define S62X_IDX_OS_X           0x16
#define S62X_IDX_OS_Y           0x17
#define S62X_IDX_OS_Z           0x18
#define S62X_IDX_OS_POL         0x19
...
#define ECO1_DEFAULT(x)         (((x)&I2C_MASK_FLAG)!=UMAP(0x1E) ? 0x40 : 0x44)
...
#define S62X_IDX_WDT_VALUE      0x38

#define S62X_IDX_EFUSE_ADDR     0x3F
#define S62X_IDX_EFUSE_CTRL     0x40
#define S62X_IDX_EFUSE_RDAT     0x41
...
static int s62x_init_flag = 0;  // 0 = OK, -1 = fail
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info s62x_init_info = {
        .name   = S62X_DEV_NAME,
...
static int ECS_InitDevice(void)
{
        ...
        if (I2C_TxData2(S62X_IDX_ECO1, ECO1_DEFAULT(this_client->addr)) < 0) {
                err_desc = "ECO1";
                goto end_of_func;
        }
        ...
        if (atomic_read(&data->info) == DEVICE_INFO_M1001) goto end_of_func;

        /* the following is for M1301 only */

        if (I2C_TxData2(S62X_IDX_GAIN_X, GAIN_DEFAULT) < 0) {
                err_desc = "GAIN_X";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_GAIN_Y, GAIN_DEFAULT) < 0) {
                err_desc = "GAIN_Y";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_GAIN_Z, GAIN_DEFAULT) < 0) {
                err_desc = "GAIN_Z";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_OS_X, 0x00) < 0) {
                err_desc = "OS_X";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_OS_Y, 0x00) < 0) {
                err_desc = "OS_Y";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_OS_Z, 0x00) < 0) {
                err_desc = "OS_Z";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_OS_POL, 0x00) < 0) {
                err_desc = "OS_POL";
                goto end_of_func;
        }

        if (I2C_TxData2(S62X_IDX_WDT_VALUE, 0x51) < 0) {
                err_desc = "WDT_VALUE";
                goto end_of_func;
        }
...
#endif
        if (id1 != DEVICE_ID_VALUE || (id2 != DEVICE_INFO_M1001 && id2 != DEVICE_INFO_M1301)) {
...
{
        static const char *supported_chip[] = { "S628", "S623", "S625" };
...
/*----------------------------------------------------------------------------*/
#if FORCE_SENSORS_LAYOUT == 0
static int efuse6 = 0;
#define LAYOUT_EFUSE_INDEX      6       //1
#define LAYOUT_EFUSE_BIT        0x40    //0x04
#define EFUSE_RDAT_MASK         0xC0    //0x0F
#define LAYOUT_BIT_IS_ZERO(x)   (((x) & LAYOUT_EFUSE_BIT) == 0)
#define LAYOUT_FOR_S613         0x8000
#endif
static int l2cvti(int layout)
{
#if FORCE_SENSORS_LAYOUT == 0 || FORCE_SENSORS_LAYOUT == 2
        static int remap[] = { 6, 7, 4, 5, 2, 3, 0, 1 };
#endif
        int cvti = layout;
        if (cvti < 0 || cvti > 7) {
                printk(KERN_ERR "S62X invalid layout (%d), force to 0\n", layout);
                cvti = 0;
        }
        ...
        if (efuse6 == 0) {
                char val;
                I2C_TxData2(S62X_IDX_EFUSE_ADDR, LAYOUT_EFUSE_INDEX << 3);
                I2C_TxData2(S62X_IDX_EFUSE_CTRL, 1);            // trigger read
                I2C_TxData2(S62X_IDX_EFUSE_CTRL, 0);            // end of trigger

                val = S62X_IDX_EFUSE_RDAT;
                if (I2C_RxData(&val, 1)) {
                        printk(KERN_ERR "S62X_IDX_EFUSE_RDAT %s fail\n", __func__);
                        val = 0; //efuse is 0 by default
                }
                efuse6 = val | ~EFUSE_RDAT_MASK;
                //printk(KERN_ERR "S62X eFuse6=0x%X\n", efuse6);
        }
        else if (efuse6 == LAYOUT_FOR_S613) {
                //int remap[] = { 2, 3, 0, 1, 6, 7, 4, 5 };
                return cvti ^ 2;
        }
        if (LAYOUT_BIT_IS_ZERO(efuse6)) return cvti;
#endif
...
{
        char strbuf[S62X_BUFSIZE];
        ECS_ReadChipInfo(strbuf, S62X_BUFSIZE, 1);
...
static ssize_t show_layout_value(struct device_driver *ddri, char *buf)
{
        ...
        struct s62x_i2c_data *data = i2c_get_clientdata(client);
#if FORCE_SENSORS_LAYOUT == 0
        int fsl = (efuse6 == LAYOUT_FOR_S613) ? 0 : (2 - LAYOUT_BIT_IS_ZERO(efuse6));
#else
        int fsl = -FORCE_SENSORS_LAYOUT;
#endif
        return sprintf(buf, "(%d, %d, %+2d)\n[%+2d %+2d %+2d]\n[%+2d %+2d %+2d]\n",
                       data->hw->direction, atomic_read(&data->layout), fsl,
...
static long s62x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        ...
        case ECS_IOCTL_READ:
                ...
                if (rwbuf[0] == 0) {
                        if (rwbuf[1] == 1 || rwbuf[1] == 2) {
                                struct s62x_i2c_data *data = i2c_get_clientdata(this_client);
                                uint32_t layout;
                                if (rwbuf[1] == 2) {
        ...
        case ECS_IOCTL_GET_PROJECT_NAME:
                ...
                status = ECS_ReadChipInfo(buff + sizeof(PROJECT_ID), S62X_BUFSIZE - sizeof(PROJECT_ID), 1);
                if (status > 0) {
        ...
        case MSENSOR_IOCTL_READ_CHIPINFO:
                ...
                ECS_ReadChipInfo(buff, S62X_BUFSIZE, 0);
...
/*----------------------------------------------------------------------------*/
static int s62x_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
static int s62x_orientation_operate(void* self, uint32_t command, void* buff_in, int size_in,
                             void* buff_out, int size_out, int* actualout)
{
        int err = 0;
        int value;
        ...
        case SENSOR_GET_DATA:
                ...
                break;
        default:
                printk(KERN_ERR "S62X osensor operate function no this parameter %d!\n", command);
                err = -1;
                break;
        }
...
{
        uint8_t loop_i;
        char cmd[] = { S62X_IDX_ECO1, ECO1_DEFAULT(client->addr) };
        struct i2c_msg msg[] = {
                {
...
static int s62x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if FORCE_SENSORS_LAYOUT == 0
                        if (atomic_read(&data->info) != DEVICE_INFO_M1001) efuse6 = LAYOUT_FOR_S613;
#endif
        ...
        if ((err = s62x_create_attr(&(s62x_init_info.platform_diver_addr->driver))) != 0) {
...
static int s62x_i2c_remove(struct i2c_client *client)
{
        ...
        if ((err = s62x_delete_attr(&(s62x_init_info.platform_diver_addr->driver))) != 0) {
...
{
        struct mag_hw *hw = get_cust_mag_hw();
        printk(KERN_INFO "S62X s62x_init\n");
        i2c_register_board_info(hw->i2c_num, &i2c_s62x, 1);
        hwmsen_msensor_add(&s62x_init_info);
        return 0;
}
...
{
        printk(KERN_INFO "S62X s62x_exit\n");
}
